...
import MostPopularPost from "../Home/GetPosts/MostPopularPost";

const CategoryPage = ({post, recentPost, popularPost}) => {
    ...
    <div className="min-h-screen bg-white text-gray-800">
      ...
      <section className="container mx-auto px-4">
        <div className="bg-white border rounded-lg p-6 shadow-sm grid grid-cols-1 md:grid-cols-2 gap-5">
          <div>
               <Link href={`/${firstPost.cname}/${firstPost.slug}`} className="text-2xl md:text-3xl font-semibold leading-snug hover:underline">
                ...
                <div className="flex items-center gap-3 mt-4 text-sm">
                  ...
                  <Link href={`/${firstPost.cname}/${firstPost.slug}`} className="bg-gray-200 text-gray-700 px-2 py-0.5 rounded text-xs"> {firstPost.authorName}</Link>
                </div>
          ...
          <div className="mt-4">
            <Link href={`/${firstPost.cname}/${firstPost.slug}`}>
                <Image
      ...
      <section className="container mx-auto px-4 mt-6 grid grid-cols-1 md:grid-cols-3 gap-6">
        {relatedPosts.map((i) => (
          <article key={i._id} className="flex gap-3 items-start border rounded p-3">
            ...
            <Link href={`/${i.cname}/${i.slug}`} className="text-sm font-medium leading-tight hover:underline">
              {i.title}
            </Link>
          </article>
        ))}
